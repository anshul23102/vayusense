...
The original GPU benchmark ran on 5,922,378 individual sensor readings pulled from the OpenAQ public archive (hosted on AWS Open Data), spanning multiple years, four Indian cities, and six pollutants (PM2.5, PM10, NO2, SO2, O3, CO). That fixed benchmark result is the honest, reproducible proof of the speedup and is not regenerated as the app grows. The live dataset has since expanded well beyond that proof of concept: it now spans twenty Indian cities (Delhi, Mumbai, Kolkata, Chennai, Bengaluru, Hyderabad, Pune, Ahmedabad, Lucknow, Patna, Jaipur, Surat, Kanpur, Nagpur, Indore, Bhopal, Visakhapatnam, Vadodara, Coimbatore, and Nashik), all six pollutants, and over 17 million combined raw sensor readings across three ingestion runs. Turning that raw firehose into a usable daily snapshot requires cleaning, resampling to daily means per city and pollutant, computing 7 day rolling trends, and flagging anomaly days.
